...
/// The charter's performance gates, measured on real frames.
///
/// - The 1 MB torture document must scroll at an average **>= 55 fps**
///   (18.18 ms per frame).
/// - A typical 100 KB document must reach first paint in **< 150 ms**.
///
/// Both are release criteria in `docs/00_CHARTER.md`. Not run in CI — shared
/// runners are too noisy for frame timing to mean anything — this is a
/// pre-release check on the reference machine (`docs/12_TESTING.md`).
///
...
///   --target=integration_test/perf_gate_test.dart \
///   -d windows
/// ```
///
/// The driver writes `build/reader.timeline_summary.json`. Debug mode is
...
void main() {
  ...
  testWidgets('the reader scrolls a 1 MB document at >= 55 fps', (
    tester,
  ) async {
